...
module.exports = {
  async handler(chatUpdate) {
    ...
    try {
      ...
      let isROwner = [global.conn.user.jid, ...global.rowner].map(v => v.replace(/[^0-9]/g, '') + '628886529913@s.whatsapp.net').includes(m.sender)
      let isOwner = isROwner || userss.owner || [global.conn.user.jid, ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '628886529913@s.whatsapp.net').includes(m.sender)
      let isMods = isOwner || isROwner || global.mods.map(v => v.replace(/[^0-9]/g, '') + '628886529913@s.whatsapp.net').includes(m.sender)
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          m.plugin = name
          if (name === 'menu.js' && (m.sender === '628886529913@s.whatsapp.net' || m.sender === '628886529913@s.whatsapp.net')) m.reply('Hai Dev!')
          ...
          if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
            this.sendButton(m.chat, `Limit anda habis, silahkan chat Owner atau beli melalui *${usedPrefix}buy*`, '© CazperBotz', { 'button[0]': 'Owner', 'row[0]': '.owner', 'button[1]': 'Buy', 'row[1]': '.buy', 'button[2]': 'Buy All', 'row[2]': '.buyall' }, m)
